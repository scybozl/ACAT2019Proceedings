...
The amplitude at any phase-space point $M_{\lambda}(\hat{s}, \hat{t})$ is then interfaced to \texttt{Powheg} via an interpolation framework. Note that in this way, the top-Higgs Yukawa coupling $y_t$ can easily be varied by rescaling the monomes in Eq.~(\ref{eq:2ndorder}):
